...
public class GenerateTicketAdapter {
  ...
  public Map<String, Object> callGenerateTicketRestService(
    final ActivatedJob job
  ) throws IOException {
    logger.info("Generate ticket via REST [" + job + "]");

    if (
      "ticket".equalsIgnoreCase(
          (String) job
            .getVariablesAsMap()
            .get(ProcessConstants.VAR_SIMULATE_BOOKING_FAILURE)
        )
    ) {
      ...
      CreateTicketResponse ticket = restTemplate.getForObject(
        endpoint + "/ticket",
      ...
      return Collections.singletonMap(
        ProcessConstants.VAR_TICKET_ID,
        ticket.ticketId
      );
    }
  }
